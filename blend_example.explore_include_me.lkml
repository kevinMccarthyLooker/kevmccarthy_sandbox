...
# view: users__blend_example_parent {sql_table_name:`bigquery-public-data.thelook_ecommerce.users`;;measure: count {type: count}dimension: id {type: number}dimension: first_name {}dimension: last_name {}dimension: email {}dimension: age {type: number}dimension: gender {}dimension: state {}dimension: street_address {}dimension: postal_code {}dimension: city {}dimension: country {}dimension: latitude {type: number}dimension: longitude {type: number}dimension: traffic_source {}dimension_group: created_at {type: time}}view: empty_base {sql_table_name:(select null);;}
# view: orders {    sql_table_name:bigquery-public-data.thelook_ecommerce.orders`;;      measure: count {  }    dimension: order_id {    type: number  }    dimension: user_id {    type: number  }    dimension: status {  }    dimension: gender {  }    dimension_group: created_at {    type: time  }    dimension_group: returned_at {    type: time  }    dimension_group: shipped_at {    type: time  }    dimension_group: delivered_at {    type: time  }    dimension: num_of_item {    type: number}}
...
#   join: users {
#     from: users__blend_example_parent
#     sql:full join ${users.SQL_TABLE_NAME} on false;;relationship: one_to_one #relationship: many_to_one # we manage it ourselves
#   }

#   #Start with next view having reference to parent's primary key.  Use your normal join criteria.
# #FULL JOIN REQUIRED TBD...
#   join: orders {
#     sql: full join ${orders.SQL_TABLE_NAME} on ${orders.user_id}=${users.id} ;;relationship: one_to_one #relationship: many_to_one # we manage it ourselves
#   }
#   #Continue recursively joining in another table with foreign key to prior join
#   join: order_items {
#     sql: full join ${order_items.SQL_TABLE_NAME} on ${orders.order_id}=${order_items.order_id} ;;relationship: one_to_one #relationship: many_to_one # we manage it ourselves
#   }
...
#   join: events {
#     sql: full join ${events.SQL_TABLE_NAME} on ${users.id}=${events.user_id};;relationship: one_to_one #relationship: many_to_one # we manage it ourselves
#   }
# }

# view: +users__blend_example_parent {dimension: view_name {sql:${TABLE};;}}#EXAMPLE illustrates we need to add these to the view object, not the explore join name. view: +users {dimension: view_name {sql:${TABLE};;}}
...
# explore: +blend_example_base {
#   cancel_grouping_fields: [users.view_name,orders.view_name,order_items.view_name,events.view_name]
...
#   extends: [users__blend_example_parent]
#   derived_table: {
...
#     # sql:full join (select blend_example_users.users.* from blend_example_users) users on true;;
#     # sql:${blend_example_users.SQL_TABLE_NAME} ;;
#     sql:  ;;
