...
#       derived_column: date_month                {sql:timestamp(date_trunc(date_date, month));;}
...
#       #set parpameterization as necessary for this version of derived table
#       filters: {
...
#     sql:
#     select
#     /*coalescing fields goes here */
#     -- coalesce(order_items.date_month,events.date_month,mockup_of_view_requiring_paramterization_data__daily.date_month) as date_month,
#     events.date_month,
#     --coalesce(order_items.user_country,events.user_country,mockup_of_view_requiring_paramterization_data__daily.user_country) as user_country,
#     events.user_country,

#     --coalesce(order_items.order_status) as order_status,

#     --(select as struct order_items.*)           as order_items,
#     (select as struct events.*)                 as events,
#     --(select as struct mockup_of_view_requiring_paramterization_data__daily.*)       as mockup_of_view_requiring_paramterization_data__daily,
#   from
# --                    (select 'order_items' as source                                         ,* from ${order_items_data.SQL_TABLE_NAME}) as order_items
# -- full outer join
#   (select 'events' as source                                              ,* from ${events_data.SQL_TABLE_NAME}) as events
#   --on false
# --  full outer join   (select 'mockup_of_view_requiring_paramterization_data__daily' as source,* from ${mockup_of_view_requiring_paramterization_data__daily.SQL_TABLE_NAME}) as mockup_of_view_requiring_paramterization_data__daily on false
#     ;;
...
include: "/models/data_blend_multi_explore_different_grain2.lkml"
